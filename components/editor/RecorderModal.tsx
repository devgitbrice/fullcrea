...
import { X, Video, Monitor, Loader2, AlertTriangle, Circle, Square, RefreshCw, Smartphone } from 'lucide-react';
import { useProject } from '@/components/ProjectContext';
import { useToast } from '@/components/Toast';
import { useEscapeKey } from '@/lib/hooks/useEscapeKey';

export type RecorderMode = 'camera' | 'screen';
...
export default function RecorderModal({ mode, onClose }: Props) {
  ...
  }, [mode, withMic, attachStream, refreshDevices, stopStream]);
  ...
  const saveRecording = useCallback(async (blob: Blob, mimeType: string | undefined) => {
    ...
    try {
      ...
      const prefix = mode === 'screen' ? 'ecran' : 'webcam';
      const file = new File([blob], `${prefix}-${stamp}.${ext}`, { type: blob.type || `video/${ext}` });
  ...
  const title = mode === 'screen' ? "Enregistrer l'écran" : 'Enregistrer une vidéo';
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800">
          <div className="flex items-center gap-2 text-white font-semibold text-sm">
            {mode === 'screen' ? <Monitor size={16} /> : <Video size={16} />}
        ...
        <div className="p-4 space-y-3">
          ...
          {mode === 'camera' && (
            <div className="space-y-1.5">
              ...
              >
                {devices.length === 0 && <option value="">Caméra par défaut</option>}
                {devices.map((d, i) => (
                  <option key={d.deviceId || i} value={d.deviceId}>
                    {isIphoneDevice(d.label) ? '📱 ' : ''}{d.label || `Caméra ${i + 1}`}
                  </option>
                ))}
              </select>
